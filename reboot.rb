...
unless ($?.exitstatus == 0)
  logger.debug "[#{Time.now.utc.strftime("%Y/%m/%d %H:%M:%S")}] ...RESTARTING SUPERHUB"
  begin
    ...
    logger.debug "[#{Time.now.utc.strftime("%Y/%m/%d %H:%M:%S")}] ...Something is wrong - #{e.class}: #{e.message}: #{e.backtrace}"
  end
end
